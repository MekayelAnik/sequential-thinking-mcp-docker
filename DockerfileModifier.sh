...
RUN --mount=type=cache,target=/root/.npm \\
    echo "Checking if package exists: ${SEQUENTIAL_THINKING_MCP_PKG}" && \\
...
LABEL org.opencontainers.image.description="Sequential Thinking MCP (mcp-proxy stdio<->HTTP bridge)"

# mcp-proxy and HAProxy concurrency defaults (overridable at runtime).
ENV MCP_PROXY_STATELESS=false
ENV SEQUENTIAL_THINKING_MAX_MEM_MB=4096
